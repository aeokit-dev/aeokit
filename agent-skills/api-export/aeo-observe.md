...
5. Compare only compatible records and retain AeoKit IDs so another operator can reproduce the report. When observing an experiment, attach compatible follow-up run IDs and move it to `evaluating` only with authorization.
